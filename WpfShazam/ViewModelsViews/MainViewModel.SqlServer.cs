...
namespace WpfShazam.ViewModelsViews
{
    ...
    public partial class MainViewModel
    {
        ...
        public string SwitchModeButtonText => _appService.AppSettings.IsSqlServerEnabled ? "Switch to Demo Mode" : "Switch to SQL Server Mode";        
        ...
        public void OnSqlServerTabActivated(bool isActivated)
        {
            _isSqlServerTabActive = isActivated;
            if (_isSqlServerTabActive)
            {
                _appService.AppSettings.SelectedTabName = AppSettings.SqlServerTabName;
                ...
                {                    
                    if (!LoadSongInfoListOnSqlServerTab())
                    {
                        ...
                        _appService.UpdateSqlServerEnabled(false);
                        OnPropertyChanged(nameof(SwitchModeButtonText));                        
                    }

                    // Auto-select SelectedSongInfoFromSqlServer
                    var songInfo = SongInfoListFromSqlServer.FirstOrDefault(x => x.SongUrl == _appService.AppSettings.SelectedSongUrl);
        ...
        {
            return LoadSongInfoListOnSqlServerTab(_appService.AppSettings.IsSqlServerEnabled);
        }
        ...
        private bool LoadSongInfoListOnSqlServerTab(bool isSqlServerEnabled)
        {
            try
            {
                ...
                StatusMessage = isSqlServerEnabled ? "Switched to SQL Server mode" : "Switched to Demo mode";
                return true;
            }                    
        ...
        partial void OnSelectedSongInfoFromSqlServerChanged(SongInfo? value)
        {
            if (value == null)
            {
                ...
                SqlServerWebView2Control.Source = _YouTubeHomeUri;
            ...
            else
            {
                ...
                SqlServerWebView2Control.Source = new Uri(value.SongUrl); // TODO: maybe test SongUrl is valid
        ...
        private void SwitchMode()
        {
            try
            {
                if (_appService.AppSettings.IsSqlServerEnabled)
                {
                ...
                if (LoadSongInfoListOnSqlServerTab(!_appService.AppSettings.IsSqlServerEnabled))
                {
                    _appService.UpdateSqlServerEnabled(!_appService.AppSettings.IsSqlServerEnabled);
                    UpdateSqlServerTabButtons();
                    OnPropertyChanged(nameof(SwitchModeButtonText));                    
                }
            }
        ...
        {
            IsDeleteSqlServerEnabled = _appService.AppSettings.IsSqlServerEnabled && _isSqlServerTabActive &&
